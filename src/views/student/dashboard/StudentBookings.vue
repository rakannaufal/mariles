          ...
          <div class="history-list">
            <div v-for="item in allHistoryTransactions" :key="item.id || item.order_id" 
              ...
              <div class="history-icon" :class="[getPaymentStatusClass(item.transaction_status), { 'cancelled-icon': item.isCancelled }]">
                <svg v-if="item.transaction_status === 'settlement' || item.transaction_status === 'paid'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
                <svg v-else-if="item.transaction_status === 'pending'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
